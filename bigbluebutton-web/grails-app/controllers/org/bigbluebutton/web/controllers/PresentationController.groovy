...
class PresentationController {
  ...
  def upload = {
    ...
    def expandAnimations = params.boolean('expand_animations')
    ...
    if (isPresentationMimeTypeValid) {
      if (expandAnimations && filenameExt.equalsIgnoreCase("pptx")) {
        File marker = new File(pres.absolutePath + ".expand-animations")
        try {
          if (!marker.createNewFile() && !marker.isFile()) {
            throw new IOException("Unable to create PowerPoint animation expansion marker")
          }
          log.info("PowerPoint animation expansion requested for " + presFilename + " (presId: " + presId + ")")
        } catch (IOException e) {
          log.error("Unable to create PowerPoint animation expansion marker for " + presFilename, e)
